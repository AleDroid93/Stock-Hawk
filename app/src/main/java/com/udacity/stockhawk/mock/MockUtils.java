...
public class MockUtils {
    ...
    public static ArrayList<HistoryItem> historyToArrayList(String history){
        ArrayList<HistoryItem> histToArrList = null;
        String historyLines[] = null;
        DateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");
        try {
            if (history != null || !history.equals("")) {
                histToArrList = new ArrayList<>();
                historyLines = history.split("\n");
                for (String s : historyLines) {
                    String[] historyLine = s.split(",", 2);
                    Calendar calendar = Calendar.getInstance();
                    calendar.setTimeInMillis(Long.parseLong(historyLine[0]));
                    String date = formatter.format(calendar.getTime());
                    Double stockPrice = Double.parseDouble(historyLine[1]);
                    histToArrList.add(new HistoryItem(date, stockPrice));
                }
            }
        }catch(Exception ex){
            Timber.e("ERRORE IN MOCK_UTILS: "+ ex.getMessage());
            ex.printStackTrace();
        }
        return histToArrList;
    }
}
